...
def modificar_subservicio(request, subservicio_id):
    subservicio = Subservicio.objects.get(idSubservicio=subservicio_id)
    ...
    if request.method == 'POST':
        ...
        subservicio.nombre = nombre
        subservicio.descripcion = descripcion
        subservicio.horarioApertura = horarioApertura
        subservicio.horarioCierre = horarioCierre
        subservicio.servicio_id = servicio_id
        
        if imagen:
            subservicio.imagen = imagen
        
        subservicio.save()
        return redirect('servicios')
    
    return render(request, 'modificar_subservicio.html', {'subservicio': subservicio})

def guardar_modificacion_subservicio(request, id_subservicio):
    subservicio = get_object_or_404(Subservicio, idSubservicio=id_subservicio)

    if request.method == 'POST':
        subservicio.nombre = request.POST.get('nombre')
        subservicio.descripcion = request.POST.get('descripcion')
        subservicio.horarioApertura = request.POST.get('horarioApertura')
        subservicio.horarioCierre = request.POST.get('horarioCierre')
        subservicio.save()

        return redirect('servicios')  # Redirige a la página de servicios después de guardar la modificación
    
    return render(request, 'guardar_modificacion_subservicio.html', {'subservicio': subservicio})

def eliminar_subservicio(request, subservicio_id):
    # Obtener el subservicio a eliminar
    subservicio = get_object_or_404(Subservicio, idSubservicio=subservicio_id)
    
    if request.method == 'POST':
        # Eliminar el subservicio
        subservicio.delete()
        return redirect('servicios')  # Reemplaza 'servicios' con la URL de la lista de servicios
        
    return redirect(request, 'eliminar_subservicio.html', {'subservicio': subservicio})
